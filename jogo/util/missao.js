...
async function interageMissao(jogadorAtualziado, npc) {
    ...
    if (opcao == '1') {
        try {
            //verifica se ja fez essa missao.
                const resultCheckMissao =  await checkMissaoFeita(missao, jogadorAtualziado)
                if(resultCheckMissao.lenght === 0){
                    console.log("missao nova")
                }
    ...
    async function checkMissaoFeita(missao, jogadorAtualziado) {
        try {
            const checkMissaoFeita = await db.query(`select * from missao_jogador  where id_missao  = '${missao.id_missao}' and id_jogador = '${jogadorAtualziado.id_jogador}'`)
            console.log(checkMissaoFeita.rows[0])
    ...
    async function concluiMissaoJogador(missao, jogadorAtualziado) {
        try {
            await db.query(`update  missao_jogador set concluida = true where id_missao = '${missao.id_missao} and id_jogador = '${jogadorAtualziado.id_jogador}'`);
        } catch (error) {
            console.log(error.stack);
        }
    }

...
async function acessaDigivice(jogadorAtualziado){
    ...
    try {
        ...
        const listDigimom = await (await db.query(`select * from instancia_digimon where id_digivice = '${idDigivice}'`)).rows[0]
        const listItem = await db.query(`select * from instancia_item where id_digivice = '${idDigivice}'`).rows[0]

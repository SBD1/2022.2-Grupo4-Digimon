...
d.nome, cd.tipo, m.vida_atual, m.nivel, m.vida, m.defesa, m.ataque, m.velocidade, m.id_monstro, m.id_batalha, max(a.dano) as dano_habilidade, max(def.defesa) as defesa_habilidade
from monstro m
...
left join ataque a on a.id_habilidade = dh.id_habilidade
left join defesa def on def.id_habilidade = dh.id_habilidade
group by d.nome, cd.tipo, m.vida_atual, m.nivel, m.vida, m.defesa, m.ataque, m.velocidade, m.id_monstro, m.id_batalha;